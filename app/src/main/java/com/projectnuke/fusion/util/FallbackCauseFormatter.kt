...
object FallbackCauseFormatter {
    ...
    private fun formatSingleEvent(event: com.projectnuke.fusion.llm.RuntimeFallbackEvent): String {
        return when (event.reason) {
            ...
            FallbackReason.SPECULATIVE_ENABLE_FLAG_SETTLEMENT_FAILED -> "예측 디코딩 활성화 실패"
            FallbackReason.SPECULATIVE_DISABLE_FLAG_SETTLEMENT_FAILED -> "예측 디코딩 비활성화 실패"
            FallbackReason.MTP_ENGINE_INIT_FAILED -> "MTP 초기화 실패"
            ...
            FallbackReason.ALL_CANDIDATES_EXHAUSTED -> "후보 모두 실패"
            FallbackReason.ALL_CANDIDATES_SKIPPED_RECENT_FAILURE -> "후보 모두 건너뜀"
        }
    }
